...
public:
    Move();
    Move(uint8_t from, uint8_t to, uint8_t attacterType, uint8_t attackerSide, uint8_t defenderType, uint8_t defenderSide, uint8_t flag = FLAGS::DEFAULT );

    friend bool operator==(Move right, Move left);
