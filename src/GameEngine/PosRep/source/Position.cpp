...
Position::Position(const std::string& shortFen, uint8_t enPassant, bool wlCastling, bool wsCastling, bool blCastling, bool bsCastling, float moveCtr, bool flag) {

    this->pieces = {shortFen, flag};
    ...
    this->hash = {this->pieces, this->whiteToMove(), this->wlCastling, this->wsCastling, this->blCastling, this->bsCastling};
...
void Position::move(Move move) {
    ...
    switch (move.getFlag()) {
        case Move::FLAGS::DEFAULT:
            break;

        case Move::FLAGS::PAWN_LONG_MOVE:
            this->changeEnPassant((move.getFrom() + move.getTo()) / 2);
            break;
        case Move::FLAGS::EN_PASSANT_CAPTURE:
    ...
    if (move.getFlag() != Move::FLAGS::PAWN_LONG_MOVE) {
...
void Position::addPiece(uint8_t square, uint8_t type, uint8_t side) {
    if (!BitBoardOperations::getBit(this->pieces.getPieceBitBoard(side, type), square)) {
