...
ZobristHash::ZobristHash(Pieces pieces, bool blackToMove, bool wlCastling, bool wsCastling, bool blCastling, bool bsCastling) {
    ...
    for (uint8_t square = 0; square < 64; square = square + 1) {
        if (BitBoardOperations::getBit(pieces.getSideBitBoard(SIDE::WHITE), square)) {
            side = SIDE::WHITE;
        }
        else if (BitBoardOperations::getBit(pieces.getSideBitBoard(SIDE::BLACK), square)) {
            side = SIDE::BLACK;
        }
        else {
            continue;
        ...
        for (uint8_t type = 0; type < 6; type = type + 1) {
            if (BitBoardOperations::getBit(pieces.getPieceBitBoard(side, type), square)) {
